...
def run_hook() -> None:
    ...
    instructions = (
        f"MANDATORY: You MUST call AskUserQuestion BEFORE taking any other action. "
        f"Do NOT write allow files, modify config, or re-run the command until the user answers.\n"
        f"\n"
        f"AskUserQuestion parameters:\n"
        f"  header: \"{base}\"\n"
        f"  question: \"`{cmd}` was flagged by BashBouncer. How should it be handled?\"\n"
        f"  options:\n"
        ...
        f"\n"
        f"AFTER the user answers, take the corresponding action:\n"
        f"- Allow once: use the Write tool to create file {allow_once_path} with content \"1\", "
        f"then re-run the original command.\n"
